...
const columns: ColumnDef<Row>[] = [
  { accessorKey: "time", header: "time", cell: ({ getValue }) => {
      const v = getValue<string>();
      try { return new Date(v).toLocaleString(); } catch { return v; }
    }
  },
  { accessorKey: "ax", header: ({ column }) => (
      <button className="select-none" onClick={() => column.toggleSorting(column.getIsSorted() === "asc")}>ax</button>
    ), cell: ({ getValue }) => getValue<number | null>() ?? "" },
  { accessorKey: "ay", header: ({ column }) => (
      <button className="select-none" onClick={() => column.toggleSorting(column.getIsSorted() === "asc")}>ay</button>
    ), cell: ({ getValue }) => getValue<number | null>() ?? "" },
  { accessorKey: "az", header: ({ column }) => (
      <button className="select-none" onClick={() => column.toggleSorting(column.getIsSorted() === "asc")}>az</button>
    ), cell: ({ getValue }) => getValue<number | null>() ?? "" },
];

export function IpiDashboard() {
      ...
      <CardContent className="space-y-4">
        <div className="w-fit">
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="space-y-2 lg:col-span-2">
              ...
              <Input id="inclinometer" value={inclinometer} onChange={(e) => setInclinometer(e.target.value)} />
            </div>

            <div className="space-y-2 lg:col-span-1">
              <Label htmlFor="node">Node</Label>
              <Input id="node" type="number" value={node} onChange={(e) => setNode(Number(e.target.value))} />
            </div>
        ...
        <div className="flex items-center gap-3">
          <Button onClick={runQuery} disabled={loading}>
            {loading ? "Querying..." : "Search database"}
          </Button>
          <div className="text-sm text-muted-foreground">Returned: {rows.length} rows</div>
        </div>
