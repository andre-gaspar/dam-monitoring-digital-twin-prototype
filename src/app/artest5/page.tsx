...
function ModePanel({
  ...
  lockButtonRef: React.RefObject<THREE.Mesh | null>;
}) {
  ...
  return (
    <group
      ref={panelRef}
      position={[0, FLOOR_OFFSET_Y + 0.88, -0.78]}
      rotation={[0, 0, 0]}
    >
      <group rotation={[MODE_PANEL_PITCH, 0, 0]}>
        ...
        <ModeButton
          ...
          active={isLocked}
          pressed={pressedButtonId === "lock"}
          position={[0.405, 0.105, 0.02]}
...
function ARScene({
  ...
  const lockButtonRef = useRef<THREE.Mesh | null>(null);
  ...
  function pulseButton(buttonId: string) {
    if (buttonId === "lock") {
      setIsMechanicsPanelLocked((value) => !value);
    }
