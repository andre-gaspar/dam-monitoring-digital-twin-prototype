...
// These are intentionally near floor-level.
// If your Quest origin behaves like eye-level, add a negative FLOOR_OFFSET_Y.
...
function ModePanel({
  ...
  return (
    <group position={[0.58, FLOOR_OFFSET_Y + 0.24, -0.95]} rotation={[0, -0.35, 0]}>
...
function HandInputSystem({
  ...
  function findClosestPointerTarget(handPointer: any) {
    ...
    for (const target of draggableTargets) {
      ...
      const intersections = handPointer.intersectObject(object, true);
  ...
  function updateRayDragMode() {
    ...
    for (const handPointer of handPointersRef.current) {
      const { closestTarget, closestDistance } = findClosestPointerTarget(handPointer);
  ...
  function getTargetContainsPoint(target: DraggableTarget, point: THREE.Vector3) {
    ...
    tmpBox.current.setFromObject(object);
...
function ARScene({
  ...
  return (
    <>
      ...
      <DraggableDrone droneRef={droneRef} />
      ...
      >
        {mode === "ray-drag"
          ? "Point + pinch to drag chart or drone model"
          : "Pinch directly on chart/model. Other hand pinch scales."}
      </Text>
    </>
  );
}
