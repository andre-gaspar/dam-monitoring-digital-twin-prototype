...
const LAKEBED_MODEL_URL = "/montesinholayersDecim3.glb";

// Camera transition speed.
// Lower = slower, higher = faster.
const CAMERA_LERP_SPEED = 4.5;

// Water playback speed.
// Lower = faster. Higher = slower.
const PLAYBACK_INTERVAL_MS = 90;//35
...
// Your current water placement looks close to the maximum visible level.
// These values map the foreign Baells data into a small visual movement range.
// Max sensor value -> original water level.
// Min sensor value -> slightly lower water level.
const WATER_VISUAL_MIN_Y = WATER_LEVEL_Y - 7.0;//2.2
const WATER_VISUAL_MAX_Y = WATER_LEVEL_Y + 0.5;//0
...
function mapWaterLevelToSceneY(
...
function Scene({
  ...
  return (
    <group ref={groupRef} position={[0, -2, 0]}>
      ...
      />
      <VertexColorModel
        url={LAKEBED_MODEL_URL}
        modelRef={lakebedModelRef}
      />
    ...
    <div style={{ width: "100vw", height: "100vh", position: "relative" }}>
      ...
      >
        ...
        >
          ...
          >
            <div style={{ fontSize: 10, opacity: 0.65 }}>Sensor level</div>
          ...
          >
            <div style={{ fontSize: 10, opacity: 0.65 }}>Scene water Y</div>
            <div style={{ fontSize: 22, fontWeight: 800, marginTop: 2 }}>
              {dynamicWaterLevelY.toFixed(2)}
            </div>
          </div>
        </div>

        <div style={{ fontSize: 11, opacity: 0.7, minHeight: 18 }}>
          {currentPlaybackRow
        ...
        <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
          <button
            ...
            disabled={waterLevels.length === 0 || isWaterLoading}
            style={{
              flex: 1,
              padding: "8px 10px",
              border: "none",
              borderRadius: 10,
              background:
                waterLevels.length === 0 || isWaterLoading ? "#777" : "white",
              color: "black",
              cursor:
                waterLevels.length === 0 || isWaterLoading
                  ? "not-allowed"
                  : "pointer",
              fontWeight: 800,
              opacity: waterLevels.length === 0 || isWaterLoading ? 0.5 : 1,
            }}
          >
